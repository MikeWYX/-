...
void addNewCmd(	unsigned char *cmd, 
    ...
    */
    struct cmd *newCmd = (struct cmd *)my_malloc(cmd_size);
	if (newCmd == NULL) {
		myPrintf(0x7,"error\n");
		return;
	}

	myStrncpy(newCmd->cmd, cmd, myStrlen(cmd)+1);
    //myPrintf(0x7,"%s\n",cmd);
    //myPrintf(0x7,"%s\n",newCmd->cmd);
	...
	myStrncpy(newCmd->description, description, myStrlen(description)+1);
	//newCmd->description[sizeof(newCmd->description) - 1] = '\0'; 
...
int help(int argc, unsigned char **argv){
    ...
    while (tmpCmd != NULL) {            
        if (my_strcmp(argv[1],tmpCmd->cmd)==0) {
